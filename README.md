...
- [ ] 波打つ板 + 高さでグラデ（Varying）— 頂点の y を `sin`/ノイズで動かし、変位量を `@location` でフラグメントへ補間して色に。頂点シェーダーが主役になる感覚と Varying をここで掴む。ガウス `exp(-x²)` を波の formula に差し替えれば凸凹の重み付けも同じ枠で扱える（→ Maxime Heckel 記事）
