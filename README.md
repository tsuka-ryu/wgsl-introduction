...
- [x] 02 映進 — せん断菱形のシェブロン（本 3.2）。**菱形は「描く形」ではなく「マスの形」** — すきまが無く全面がどれかの菱形に属するので、必要なのは形ではなく**平面の分割**。座標をせん断すれば（`row = floor(st.y + SLOPE*lx)`）同じ `floor` でも境界線が斜めになり、平行四辺形の格子が勝手にできる（菱形の SDF は1つも要らない = 4.6「タイル⇔線の双対」の実践）。列ごとに `lx = 1 - lx` でせん断の向きを反転（縦軸で鏡映）してシェブロンに。★**映進は「形」ではなく「色の並び」に入っていた** — 折れ線は列をまたいで連続していてずれておらず、繰り返しの1単位は本の赤枠どおり**1列 × 2段**（1列の中では2色が交互、隣の列が別の2色ペアで4色）。★**鏡映があるおかげで角度と高さが独立に選べる** — 鏡なしの格子だと「1列で1段」ぴったり上がらないと線が繋がらないが、列ごとに反転していると境界が必ず折り返し点（V の頂点）になるので `SLOPE` は任意でよく、太さ=`ROWS` / 角度=`SLOPE` と分離できる
