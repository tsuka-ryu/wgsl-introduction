...
- [x] 「幾何学パターンづくりのすべて — ファッション、建築、デザインのためのリピートパターン制作ガイド」— 座標変換の柱の体系化として読む。3章「平面対称」の17節はおそらく**壁紙群が17種類**あることに対応（平面充填リピートの対称性は並進・回転・鏡映・映進の組合せで厳密に17通り）。シェーダー的には各対称群 = 平面を基本領域に折りたたむ純関数で、**17個の壁紙群 = 17個の座標変換コンビネータ** — シェーダー生成言語の長期目標的にも群論がそのままコンポジションの API になる構造。既習との対応: 並進=`fract` / 鏡映=`abs`・三角波折り返し / 回転対称=極座標の角度 `mod` 扇形折りたたみ / 映進=並進∘鏡映（未実装の道具）。デザイナー向けでコードは無いので、Unity デッキと同じく「この対称操作は WGSL でどう書くか」を都度このリポジトリで再実装しながら読む。5章シームレス/エッシャー型リピートは truchet・タイリングの記憶があるうちだと効果的。**Part 1-5 読了**。読書メモは [`src/geometric-patterns/note.md`](src/geometric-patterns/note.md)、実装は `src/geometric-patterns/01`〜`06`（ロゼット / 万華鏡 / ルーン / フリーズ群7種 / ドット格子 / タンブリングブロック）。**得た芯**: パターン = 座標の純関数で、対称操作 = 座標の畳み方（並進=`fract` / 鏡映=`abs` / 回転=極座標 `mod` / 映進=`fract`∘`abs`）。制約が数を決める構造の反復（4対称操作 → 7フリーズ群 → 17壁紙群 → 単独充填3 → 半正8）で、土台は結晶学的制限（回転は 2,3,4,6 のみ）。実装の分かれ目は**鏡ありなら基本領域に `abs` で畳む（万華鏡）／鏡なしはキラルに回転コピー**

### 平面リピートパターン（`src/repeat-patterns/`）

本を読み終えたので、次は**1枚の絵を仕上げる**方向に使う。気に入った柄（本の図版・和柄・織物など）を見て「これはどの対称操作の組合せか」を読み解き、同じ要領で1本ずつ実装していくトラック。新プリミティブを増やすのが目的ではなく、**合成の型を集める**のが目的（[`docs/denotational-shader-language.md`](docs/denotational-shader-language.md) のコンビネータ設計の素材）。

- [x] 01 並進 — 4色ドットの2×2セル（本 3.1）。対称操作は **p1（並進だけ）**なのに単純なタイリングに見えないのは、繰り返しの1単位が円1個ではなく**円 2×2 個**で、そこに4色を割り当てているから。要点は `st = floor(st) + fract(st)` の**両側を使う**こと — **形は畳んだ側（`fract`）、色は捨てた側（`floor` = セル番号の偶奇）**に依存させる分業。全マス同じ色なら `05-dot-lattice` と同じただの p1 で、**`floor` を色に使った瞬間に繰り返しの単位が 2×2 に広がる**。輪郭は `fwidth(d)` をぼかし幅にした `smoothstep` で解像度非依存のアンチエイリアス
- [x] 02 映進 — せん断菱形のシェブロン（本 3.2）。**菱形は「描く形」ではなく「マスの形」** — すきまが無く全面がどれかの菱形に属するので、必要なのは形ではなく**平面の分割**。座標をせん断すれば（`row = floor(st.y + SLOPE*lx)`）同じ `floor` でも境界線が斜めになり、平行四辺形の格子が勝手にできる（菱形の SDF は1つも要らない = 4.6「タイル⇔線の双対」の実践）。列ごとに `lx = 1 - lx` でせん断の向きを反転（縦軸で鏡映）してシェブロンに。★**名前に反して映進は入っていない** — 折れ線が列をまたいで連続し、列の境目に縦の鏡線が生きているので、骨格は**二重鏡映（3.3）**。本物の映進は `+ 0.5*(col%2)` を足して頂点をずらした pg だが、そちらは本の図版と違って見えたので見た目を採った。★さらに**色まで含めると残る対称操作は並進だけ（p1）** — 鏡で写すと偶数列が奇数列に移るが、両者は別の色ペアなので色が保たれない。**本が言う対称操作は「モチーフの配置＝骨格」の話で、全部違う色に塗った瞬間どの図も p1 に落ちる**（色は対称性を壊す方向に効く道具。01/02 が並進だけなのに単純に見えないのはこれ）。繰り返しの1単位は本の赤枠どおり**1列 × 2段**（1列の中では2色が交互、隣の列が別の2色ペアで4色）。★**鏡映があるおかげで角度と高さが独立に選べる** — 鏡なしの格子だと「1列で1段」ぴったり上がらないと線が繋がらないが、列ごとに反転していると境界が必ず折り返し点（V の頂点）になるので `SLOPE` は任意でよく、太さ=`ROWS` / 角度=`SLOPE` と分離できる
- [x] 03 鏡映 + 平行映進 — 角丸タイル（本 3.4）。**格子は 02 の使い回し**（せん断 + 列ごとの鏡映で平行四辺形のセル）で、違いはベタ塗りの代わりに**セル内の距離場**を作って角を丸め、縮んだ分を地（白）で埋めたこと。★02 は分割だったので AA が効かなかったが、**距離場を持たせた瞬間に効く = 分割と形は行き来できる**。★**角丸 = 縮めて → 太らせる**（距離場から定数を引くとその距離だけ外へ膨らみ、尖った角は頂点を中心とする円弧になる）。ただし太らせる前の距離場が角の外側で「頂点までの距離」になっている必要があり、`max(d1,d2)` のままでは何度太らせても角が丸まらない。★セルは2辺が直交しないので `roundedBox` の `length(max(q,0))` が使えず、法線の内積 `cs` から `√(d1²+d2²−2·cs·d1·d2)/√(1−cs²)` で頂点距離を復元（直交 `cs=0` で `length(max(q,0))` に退化する一般化）。切り替えは「両方正」ではなく**法線コーンの中** `d1 > cs·d2 && d2 > cs·d1`（両方正だと値が飛んで輪郭に折れ目）。★`fwidth` はセル境界で `d` が飛ぶため誤検出して地にゴミが乗るので `aa` は自前計算、ResizeObserver は `device-pixel-content-box` で実ピクセル描画。★★**また対称操作は名前ほど実装に入っていない（02 に続き2回目）** — 実装に入ったのは鏡映（`lx = 1 - lx`）と並進だけで、「平行映進」は 02 と同じく色の並び（`k = (col + 2*row) % 4` の列ごとに半周期ずらす）に吸収されている。4色とも違うので色込みの対称群はまた p1。**本の分類はモチーフ配置＝骨格の話で、塗り分けた瞬間 p1 に落ちる**が2例目
- [ ] 04 以降 — 描きたい絵が出てきた時点で追加する（必要性が先、道具は後。先回りして候補を並べない）
