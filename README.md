...
- [ ] 🌴 **卒業制作: Vaporwave シーン** — [Maxime Heckel「Vaporwave 3D scene with Three.js」](https://blog.maximeheckel.com/posts/vaporwave-3d-scene-with-threejs/) を WGSL で再構成。波打つ板を地形に育て、グリッド線 + フォグ + RGBShift ポストプロセスで 80s 看板に。§4 の変位に、AA（グリッド線）・色空間（RGBShift/ガンマ）・ライティングを合流させる総仕上げ。※原典は Three の `displacementMap` 任せで頂点シェーダーを書いていないので、WGSL 版はむしろ本物の頂点変位に格上げできる
...
- [ ] 01 GLSL だけでレンダリングする / 02 時間とマウス座標 / 03 オーブのレンダリング / 04 様々な図形を描く — ここまでは今までの `色 = f(uv, time)` の復習

**フラクタル・ノイズ（既習トピックの GLSL 版）**

- [ ] 05 マンデルブロ集合 / 06 ジュリア集合 / 07 フラグメントシェーダ ノイズ — フラクタルとノイズを GLSL で。エスケープ時間 `zₙ₊₁=zₙ²+c` の複素数フラクタルはここが初出（この反復は後の compute 編 buddhabrot で scatter 視点として再利用）、ノイズは BoS 11章の復習

**レイマーチング本体（ここから3D・新概念）**

- [ ] 08 シェーダ内でレイを定義する — 各ピクセルからレイを飛ばす。3Dの入り口
- [ ] 09 レイマーチングで球体を描く — SDF `length(p)−r` に沿ってレイを進める中核アルゴリズム
...
- [ ] 12 オブジェクトの複製 repetition — `fract`/`mod` で空間を折って無限複製。BoS 9章パターンの3D版
