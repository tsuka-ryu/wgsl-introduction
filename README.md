...
- [x] 15 重なりを考慮した描画（`min` 合成）([`src/raymarching/15-union`](src/raymarching/15-union/main.ts)) / 16 補間して結合（smooth min）— BoS 12章の距離場 `min`/`max` 合成が3Dに。15 はトーラス∪床を `min(d1,d2)` で合成(CSG 和集合の初出)
