...
//   シェブロンになる。折れ線は列をまたいで連続していて、列の境目に縦の鏡線が
//   生きている → 骨格は二重鏡映 (本の 3.3) で、映進 (3.2) ではない。
//   映進は「鏡映してから鏡の軸に沿って半周期ずらす」なので、頂点が揃わずズレる:
//     let row = floor(st.y + SLOPE * lx + 0.5 * (col % 2.0));   // ← これで本物の pg
//   ずらした絵が本の図版と違って見えたので、見た目を採ってずらしていない。
//
//   さらに色まで含めると、残る対称操作は並進だけ (p1)。鏡で写すと偶数列が奇数列に
//   移るが、両者は別の色ペアを使うので色が保たれないため。★本が言う対称操作は
//   「モチーフの配置 = 骨格」の話で、全部違う色に塗った瞬間どの図も p1 に落ちる。
//   色は対称性を壊す方向に効く道具 — 01 も 02 も並進だけなのに単純に見えないのはそれ。
//
//   繰り返しの1単位は本の図の赤枠どおり「1列 × 2段」。1つの列の中では2色が交互に
//   出るだけで、隣の列が別の2色ペアを使うので合計4色になる。
//
